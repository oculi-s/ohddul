...
const Search = ({ meta, group, router, refs, userMeta, setAsideShow, view, setView }) => {
    ...
    const change = e => {
        ...
        setView(true);
        q = q.toLowerCase();
        let reg = new RegExp(`${q}|${inko.en2ko(q)}|${inko.ko2en(q)}`);
    ...
    const selectTab = e => {
        if (e.key == 'Tab' && !e.altKey && !e.shiftKey && !e.ctrlKey) {
            e.preventDefault();
            setTabIndex(1 - Tabindex);
            refs.current.text.focus();
        }
    }
    const selectNext = (e, i) => {
        if (e.key == 'ArrowDown') {
            e.preventDefault();
    ...
    const datas = [
        ...
        (<div key={1}><UserResult /></div>)
    ]
    const Result = () => {
        return (
            <div className={`${styles.result} ${view ? '' : styles.d}`}>
